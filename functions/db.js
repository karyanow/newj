...
import { get_text, formatText, formatTimeMMT } from './utils.js';

// =========================================================================
// LOW-LEVEL KV ACCESS (REQUIRES env)
// =========================================================================

/**
 * Atomically reads JSON data from KV.
 * @param {string} key KV key name
 * @param {Object} env Cloudflare environment object (to access env.BOT_KV)
 */
async function readKV(key, env) {
    try {
        // @ts-ignore: Accessing KV binding via env object
        const content = await env.BOT_KV.get(key); // <--- KV Read Operation
...
/**
 * Atomically reads, modifies, and writes JSON data back to KV.
 * @param {string} key KV key name
 * @param {Function} modifierCallback Function to modify the data read from KV.
 * @param {Object} env Cloudflare environment object (to access env.BOT_KV)
 */
async function updateKV(key, modifierCallback, env) {
    try {
        const data = await readKV(key, env);
        const updatedData = modifierCallback(data);
        // @ts-ignore: Accessing KV binding via env object
        await env.BOT_KV.put(key, JSON.stringify(updatedData)); // <--- KV Write Operation
        return true;
    } catch (e) {
...
// =========================================================================
// USER/STATE DATABASE FUNCTIONS (ALL REQUIRE env)
// =========================================================================

/**
 * Saves the generated premium account key to the user's database entry.
 */
...
/**
 * Saves a key redeemed by credit to the separate user premium keys KV store.
 */
...
/**
 * Retrieves all redeemed keys for a user.
 */
...
/**
 * Deletes a redeemed key for a user.
 */
...
export async function saveUser(userId, username, firstName, lastName = '', lang = null, referrerId = null, env) {
    ...
    await updateKV(BOT_USERS_KEY, (users) => {
        if (!users[userKey]) {
            ...
            // Ensure compatibility for older records
...
/**
 * Atomically modifies user's credit balance and verification status, and logs transaction.
 */
...
/**
 * Cleans the referrer ID string from the KV entry, ensuring it's a clean integer ID.
 */
...
/**
 * Awards credit to the referrer when the referred user verifies channel join.
 * NOTE: This function only updates KV and returns the notification data.
 * The handler calls sendMessage.
 */
export async function awardReferralCreditOnVerify(referredUserId, env) {
    ...
    // Mark referred user's flag to prevent double award AND increment referred_count for referrer
    await updateKV(BOT_USERS_KEY, (users) => {
    ...
    // Return notification data for handler to send message
    if (creditSuccess) {
...
/**
 * Set user's language explicitly.
 */
...
/**
 * Get user statistics (now includes sorting by last_active).
 */
...
/**
 * Get user activity statistics based on last_active time for the Usage Report.
 */
export function getUserActivityStats(users) {
    ...
    stats.year = stats.total; // Total users is the annual count assumption
    return stats;
}
...
/**
 * Atomically reads user state data.
 */
...
/**
 * Atomically sets user state data.
 */
